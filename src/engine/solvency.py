...
# ── 2. Scorers — cada um entrega contrib_* pronta ─────────────────────────────
...
df["ccauc"]           = df["ccauc"].fillna(1.0)        # ausente no CAUC → pior caso
...
print(f"  Crônicos estruturais (≥5 anos): {(df['n_anos_cronicos'] >= 5).sum()}")
...
# Slot extensível: adicionar futuras penalidades aqui antes do sum()
...
COLS_DEBUG = ["ente", "score", "classificacao", "anos_entregues",
