"""
pncp_aggregator.py — Enriquece o score de solvência com dados de mercado do PNCP.
SolveLicita | Fase 0

Responsabilidades:
1. Ler pncp_licitacoes_pb.csv (54k linhas brutas)
2. Agregar por município: volume, valor homologado, padrão de compra, temporalidade
3. Fazer merge com score_municipios_pb.csv
4. Exportar score_municipios_pb_pncp.csv para uso no Streamlit e no relatório
...
# Valor homologado de dispensas (NaN → 0 para soma)
...
# Coluna de diagnóstico: alto volume de dispensa em município de risco
# (usado no relatório — não no score)
...
# B — Exposição em municípios com caixa negativo
scaixa_neg = merged[merged["scaixa_medio"] < 0]
print(f"\n  🔴 Scaixa negativo ({len(scaixa_neg)} municípios):")
print(f"     Valor homologado : "
      f"R$ {scaixa_neg['valor_homologado_total'].sum():,.0f}")
print(f"     Licitações       : "
      f"{scaixa_neg['n_licitacoes'].sum():,.0f}")

# C — Invisíveis ativos (Sem Dados + PNCP)
sem_dados = merged[
...
# D — Alerta dispensa em municípios de risco
alerta = merged[
...
print(f"\n✅ Exportado: {OUT.name}")
print(f"   {len(merged)} municípios | "
      f"{len(merged.columns)} colunas")
print("=" * 65)
