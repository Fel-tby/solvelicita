...
// Colunas da tabela com config de sort
const COLUNAS = [
  { key: '#',         label: '#',            field: null,                    num: true  },
  { key: 'ente',      label: 'Município',    field: 'ente',                  num: false },
  { key: 'score',     label: 'Score',        field: 'score',                 num: true  },
  { key: 'class',     label: 'Risco',        field: 'classificacao',         num: false },
  { key: 'pop',       label: 'Pop.',         field: 'populacao',             num: true  },
  { key: 'eorcam',    label: 'Exec.Orç.%',   field: 'eorcam_raw',            num: true  },
  { key: 'rproc',     label: 'RP Proc.',     field: 'n_anos_cronicos',       num: true  },
  { key: 'siconfi',   label: 'SICONFI',      field: 'qsiconfi',              num: true  },
  { key: 'cauc',      label: 'CAUC',         field: 'ccauc',                 num: true  },
  { key: 'lliq',      label: 'Lliq',         field: 'lliq_raw',              num: true  },
  { key: 'autonomia', label: 'Autonomia',    field: 'autonomia_media',       num: true  },
  { key: 'licit',     label: 'Licitações',   field: 'n_licitacoes',          num: true  },
  { key: 'valor',     label: 'Val.Homolog.', field: 'valor_homologado_total',num: true  },
  { key: 'dispensa',  label: '% Dispensa',   field: 'pct_dispensa',          num: true  },
  { key: 'alertas',   label: 'Alertas',      field: null,                    num: false },
]
...
function KPI({ label, value, destaque }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)',
      ...
      padding: '10px 14px', borderRadius: '3px', flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: '0.6rem', color: 'var(--text-lo)', textTransform: 'uppercase',
        letterSpacing: '0.1em', fontFamily: 'JetBrains Mono, monospace', marginBottom: '3px' }}>
        {label}
      </div>
      <div style={{ fontSize: '1.6rem', fontWeight: 700, color: 'var(--text-hi)',
...
export default function Dashboard() {
  ...
  // Sort state: default = classificação → score (respeitando caps)
  ...
  const municipiosFiltrados = useMemo(() => {
    ...
    return [...filtrados].sort((a, b) => {
      ...
      if (sortField === 'classificacao') {
        // Ordenação primária: ordem de risco (caps respeitados)
        // Secundária: score decrescente dentro de cada classe
        ...
        // Dentro da mesma classe: score decrescente sempre
    ...
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden',
          padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '10px', minWidth: 0 }}>

          <div style={{ borderLeft: '3px solid var(--accent)', paddingLeft: '11px' }}>
            <h1 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-hi)',
          ...
          {/* Mapa + Painel */}
          <div style={{ display: 'flex', gap: '10px', height: '480px', minHeight: '480px' }}>

            <div style={{ flex: '1 1 0', minWidth: 0, background: 'var(--bg-panel)',
            ...
            {/* Painel direito */}
            <div style={{ width: '270px', minWidth: '270px', display: 'flex',
          ...
          {/* Tabela com sort clicável e preview colapsável */}
          <div style={{ flexShrink: 0, background: 'var(--bg-panel)', border: '1px solid var(--border)',
              ...
              <thead>
                <tr style={{ background: 'var(--bg-card)' }}>
                  {COLUNAS.map(col => {
                    ...
                    return (
                      <th key={col.key}
                        onClick={() => clicavel && handleSort(col.field)}
                ...
                {(tabelaExpandida ? municipiosFiltrados : municipiosFiltrados.slice(0, PREVIEW_LINHAS)).map((m, i) => {
                  ...
                  return (
                    <tr key={m.cod_ibge} onClick={() => setMunSelecionado(m)}
                      ...
                      <td style={{ padding: '5px 9px', color: 'var(--text-hi)', fontWeight: 500 }}>{m.ente}</td>
                      <td style={{ padding: '5px 9px', fontFamily: 'JetBrains Mono, monospace',
                        color: corPorScore(m.score), fontWeight: 700 }}>
            ...
            {municipiosFiltrados.length > PREVIEW_LINHAS && (
              <div
                onClick={() => setTabelaExpandida(prev => !prev)}
                style={{
          ...
          <div style={{ fontSize: '0.6rem', color: 'var(--text-lo)', fontFamily: 'JetBrains Mono, monospace',
            borderTop: '1px solid var(--border-dim)', paddingTop: '8px',
            display: 'flex', justifyContent: 'space-between', paddingBottom: '4px' }}>
